...
export const customContext = extendContext((interaction) => ({
  ...
  getLocale: async () =>
    interaction.client
      .t(await interaction.client.database.getLocale(interaction.guildId!))
      .get(),
}));

/**
 *
 * Mirai custom runtime configuration.
 * @returns
 */
export const miraiRC = async (): Promise<any> => {
  const { locations, debug, ...env } = await magicImport(
    join(process.cwd(), "seyfert.config.js")
  ).then((x) => x.default ?? x);
  return {
    ...env,
    debug: !!debug,
...
export const convertToHEX = (color?: number) =>
  color ? `#${color.toString(16).padStart(6, "0")}` : "#FFFFFF";
